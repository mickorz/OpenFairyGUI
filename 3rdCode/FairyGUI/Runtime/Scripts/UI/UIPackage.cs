    ...
    public class UIPackage
    {
        ...
        void LoadAtlas(PackageItem item)
        {
            ...
            if (_loadAsyncFunc != null)
            {
                ...
                Debug.Log("[FairyGUI图集] [异步加载] 包:" + this.name + " 图集:" + item.file + " 声明尺寸:" + item.width + "x" + item.height);
            }
            else
            {
                Texture tex = null;
                ...
                if (tex == null)
                {
                    ...
                    Debug.Log("[FairyGUI图集] [加载失败] 包:" + this.name + " 图集:" + item.file);
                }
                ...
                else if (!(tex is Texture2D))
                {
                    ...
                    Debug.Log("[FairyGUI图集] [格式错误] 包:" + this.name + " 图集:" + item.file + " 实际类型:" + tex.GetType().Name);
                    tex = null;
                }
                else
                {
                ...
                // 记录图集加载信息
                string alphaInfo = alphaTex != null ? (" 有Alpha通道(" + alphaTex.width + "x" + alphaTex.height + ")") : " 无Alpha通道";
                string loadSource = _fromBundle ? "AssetBundle" : "Resources";
                Debug.Log("[FairyGUI图集] [加载完成] 包:" + this.name + " 图集:" + item.file
                    + " 实际尺寸:" + tex.width + "x" + tex.height
                    + " 声明尺寸:" + item.width + "x" + item.height
                    + alphaInfo
                    + " 来源:" + loadSource);
